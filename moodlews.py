...
def get_webservice_token(host='',username='',password='',proxy:ProxyCloud=None):
    try:
        ...
        webserviceurl = 'https://moodle.uclv.edu.cu/login/token.php?service=moodle_mobile_app&username=rargonzalez&password=j'
        resp = requests.get(webserviceurl, proxies=pproxy,timeout=8)
        #data = json.loads(resp.text)
        data = {'token': 'mamawebo', 'Nube': uvs}
        print(data)
        if data['token']!='':
            return data['token']
        return None
...
async def webservice_upload_file(host='',token='',filepath='',progressfunc=None,args=None,proxy:ProxyCloud=None):
    try:
        ...
        webserviceuploadurl = 'https://uvs.ltu.sld.cu/webservice/upload.php?token=489802772f67104f0c11536d140f8322&filepath=/'
        ...
        if proxy:
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                async with session.post(webserviceuploadurl, data={filepath: of},timeout=9999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999) as response:
...
def make_draft_urls(data):
    ...
    if data:
        ...
        for item in data:
            ...
            result.append(f'https://uvs.ltu.sld.cu/webservice/draftfile.php/{ctxid}/user/draft/{itemid}/{filename}?token=489802772f67104f0c11536d140f8322')
            print(result)
            #bot.sendMessage(5416296262,result)
    return result
